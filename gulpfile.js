	...
	gulp.src('app/scss/*.scss')
		...
		.pipe(gcallback(function() {
			gulp.src('app/css/all/*.css')
			    .pipe(concatCss("_main.min.css"))
			    .pipe(minifyCss("_main.min.css"))
			    .pipe(gulp.dest('app'));
		}));
});
...
gulp.task('scss_header', function() {
	gulp.src('app/scss/header/*.scss')
		...
		.on('error', log)
		.pipe(gulp.dest('app/css/header'))
		.pipe(gcallback(function() {
			gulp.src('app/css/header/*.css')
			    .pipe(concatCss("header.css"))
			    .pipe(minifyCss("header.css"))
			    .pipe(gulp.dest('app'));
		}));
});
gulp.task('scss_fonts', function() {
	gulp.src('app/fonts/*.scss')
		.pipe(sass({
			noCache: true,
			style: "expanded",
			lineNumbers: true,
		...
		.pipe(gcallback(function() {
			gulp.src('app/css/fonts/*.css')
				.pipe(concatCss("_fonts.css"))
			    .pipe(minifyCss("_fonts.css"))
			    .pipe(gulp.dest('app'));
		}));
});
// gulp.task('scss_header', function() {
...
gulp.task('server', ['jade', 'scss', 'scss_oldIe', 'scss_fonts', 'scss_header'], function () {
	browserSync({
		notify: false,
		port: 9000,
		server: {
...
gulp.task('watch', function () {
	...
	gulp.watch('app/fonts/*.scss', ['scss_fonts']);
	gulp.watch('app/scss/header/*.scss', ['scss_header']);
